...
export default function handler(req, res) {
    ...
    got(mlhURL).then(response => {
        ...
        for (let i = 0; i < numOfHackathons; i++){
            ...
            hackathonObject['logoImageURL'] = getProperty(dom, i, eventLogoIndex);
            hackathonObject['date'] = getProperty(dom, i, eventDateIndex);
            hackathonObject['location'] = getProperty(dom, i, eventLocationIndex);
            hackathonObject['format'] =getProperty(dom, i, eventNotesIndex);
...
function getProperty(domObject, positions, propertyIndex){
    ...
    const mainContainer = domObject.window.document.getElementsByClassName('inner')[positions];
